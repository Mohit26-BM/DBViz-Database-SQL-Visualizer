...
import { bPlusTreeSteps, BPLUS_CODE, ORDER } from './engine'
  ...
  sections: [
    {
      ...
      items: [
        { label: 'Tree of order 2', text: 'Each internal node holds at most 3 keys. When a node overflows it splits.' },
        { label: 'Internal vs leaf', text: 'Blue nodes route; green nodes store the actual data records.' },
        { label: 'Linked leaves', text: 'All leaf nodes form a linked list enabling fast range scans.' },
      ],
    },
    {
      heading: 'How to run',
      items: [
        { label: 'Enter keys', text: 'Type space-separated integers in the input box then click Build Tree.' },
        { label: 'Step through', text: 'Use Play or the arrows to move through each insert and split event.' },
        { label: 'Code panel', text: 'The right panel highlights the pseudocode line executing at each step.' },
      ],
    },
  ],
}
...
function TreeView({ treeNodes, highlightNodeIds = [] }) {
  if (!treeNodes || treeNodes.length === 0)
    return <div className="flex items-center justify-center h-48 text-db-muted text-sm">Empty tree</div>
        ...
        <g transform={`translate(${svgW / 2}, 30)`}>
          ...
          {treeNodes.map(node => {
            ...
            return (
              ...
              >
                <rect x={pos.x - nodeW / 2} y={pos.y} width={nodeW} height={nodeH} rx={6}
                  fill={node.isLeaf ? (isHighlighted ? '#14532d' : '#0f2d1d') : (isHighlighted ? '#1e3a8a' : '#1e293b')}
                  stroke={node.isLeaf ? (isHighlighted ? '#4ade80' : '#22c55e') : (isHighlighted ? '#60a5fa' : '#3b82f6')}
                  strokeWidth={isHighlighted ? 2 : 1.5}
                />
                {node.keys.map((k, i) => (
                  <g key={i}>
                    {i > 0 && (
                      <line x1={pos.x - nodeW / 2 + keyW * i} y1={pos.y + 4}
                        x2={pos.x - nodeW / 2 + keyW * i} y2={pos.y + nodeH - 4}
                        stroke={node.isLeaf ? '#22c55e' : '#3b82f6'} strokeWidth={1}
                      />
                    )}
                    <text x={pos.x - nodeW / 2 + keyW * i + keyW / 2} y={pos.y + nodeH / 2 + 1}
                ...
                {node.isLeaf && (
                  <text x={pos.x} y={pos.y + nodeH + 14} textAnchor="middle" fill="#334155" fontSize={9} fontFamily="Inter, sans-serif">
                    leaf
                  </text>
                )}
              </motion.g>
...
export default function BPlusTreePage() {
  ...
  return (
    <SimulationPlayer
      title="B+ Tree Simulator"
      subtitle={`Visualize insertions and splits in a B+ tree of order ${ORDER}`}
      accentColor="purple"
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-db-bg border border-db-border text-white rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-db-purple resize-none"
            />
          ...
          {step && (
            <div className="space-y-1">
              ...
              <span className={`text-sm font-mono font-bold ${ACTION_COLOR[step.action] ?? 'text-white'}`}>
                {step.action?.replace('-', ' ')}
              </span>
              {step.key !== null && (
      ...
      renderRight={({ step }) => (
        <CodePanel code={BPLUS_CODE} highlightLine={step?.highlightLine ?? -1} />
      )}
    />
  )
}
