...
import { ACID_PROPERTIES, PROPERTY_META, acidSteps } from './engine'
...
const HELP = {
  ...
  sections: [
    {
      heading: 'Four scenarios',
      items: [
        { label: 'Atomicity', text: 'A crash mid-transaction triggers a full rollback.' },
        { label: 'Consistency', text: 'A balance-sum invariant holds before and after a transfer.' },
        { label: 'Isolation', text: 'A concurrent dirty read is prevented by the isolation level.' },
        { label: 'Durability', text: 'WAL flush before commit acknowledgement; crash recovery replays the log.' },
      ],
    },
    {
      heading: 'How to run',
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            {ACID_PROPERTIES.map(prop => {
              ...
              return (
                <button key={prop} onClick={() => { setProperty(prop); reset(acidSteps(prop)) }}
      ...
      renderCenter={({ step, currentStep }) => (
        <div className="space-y-4">
          <div className="bg-db-card border border-db-border rounded-2xl p-5">
            ...
            {step ? <TxLog log={step.txLog} /> : (
              <div className="text-center py-8 text-db-muted text-sm">Select a property and press Play</div>
            )}
          </div>
          ...
          <AnimatePresence mode="wait">
            {step?.isFailure && (
              <motion.div key="fail"
                ...
                FAILURE — {step.event.replace(/-/g, ' ').toUpperCase()}
              </motion.div>
            )}
          </AnimatePresence>
